...
````
That is about 74%. So it is kind of likely that the click rate is larger in the first case, but we are far from sure. 

[Go to the official daily data.]


```{r}
difference.samples <- 
  function (a.views, a.click, b.views, b.clicks, n.samples=100000, c.prior=1, v.prior=1) {
     a.posterior.samples <- rbeta(n.samples, a.clicks + v.prior, a.views-a.clicks + v.prior)
     b.posterior.samples <- rbeta(n.samples, b.clicks + v.prior, b.views-b.clicks + v.prior)
     b.posterior.samples - a.posterior.samples
  }
````
