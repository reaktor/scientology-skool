...
p(\theta\, |\, k, N) \propto \theta^k\, (1-\theta)^{N-k}\;p(\theta)\;.
$$
Prior knowledge about the click rate $theta$ is encoded to the prior, $p(\theta)$. Then $p(\theta)$ is not constant. A constant prior is convenient, for it leads the posterior to be the Beta distiribution, for which we have tools in many languages. 

But there is a trick available for certain kinds of non-constant priors. If prior is of the form
$$
...
Note that the this kind of prior is also a Beta distribution! You can choose $\alpha$ and $\beta$ by plotting the prior with different values, and see which fits with our preconceptions.
...
In general, a prior that melts onto the "likelihood" part of the model is called _conjugate_. Non-conjugate priors can be used, but computations is more difficult. 

Note that in objective settings, if you want to convince others, your prior is be somehow generally acceptable. Especially, if you compare groups A and B, you should not have a different prior for different groups, at least not without a very strong reason.
